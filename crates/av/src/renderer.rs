...
impl SharedNvImporter {
    ...
    /// Limpa cache de texturas importadas (troca de serviço / reset de stream).
...
impl VideoRenderer {
    ...
    /// Invalida cache de texturas NV12 compartilhadas e pool RGBA de saída (troca de serviço).
    ///
    /// SPEC-AV-006
    pub fn reset_shared_cache(&mut self) {
        #[cfg(windows)]
        self.shared_importer.reset_cache();
        // Força recriação do pool RGBA — evita frame fantasma do serviço anterior
        // quando a resolução não muda (1080i→1080i).
        self.out_pool.clear();
        self.out_dims = None;
        self.pool_idx = 0;
    }
